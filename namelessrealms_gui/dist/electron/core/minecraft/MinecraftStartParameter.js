...
class MinecraftStartParameter {
    constructor(serverAssetsObjects, mojangAssetsGameJsonObjects, ioFile) {
        this._launcherVersion = version_1.LAUNCHER_VERSION;
        this._serverAssetsObjects = serverAssetsObjects;
        this._mojangAssetsGameJsonObjects = mojangAssetsGameJsonObjects;
        this._nativesDirPath = path.join(GlobalPath_1.default.getCommonDirPath(), "bin", uuid.v4().split("-")[0]);
        this._ioFile = ioFile;
        this._instanceDirPath = path.join(GlobalPath_1.default.getInstancesDirPath(), serverAssetsObjects.id, ".minecraft");
    }
    getMinecraftJavaStartParameters() {
        let array;
        if (Utils_1.default.isMcVersion("1.13", this._serverAssetsObjects.minecraftVersion)) {
    ...
    _getJavaVMPath() {
        const builtInJavaVMDirPath = path.join(GlobalPath_1.default.getCommonDirPath(), "runtime", Utils_1.default.getOSType(), this._serverAssetsObjects.javaVM.version);
    ...
    _getBuildArray_112later() {
        let minecraftArguments = null;
        let array = new Array();
        ...
        if (this._isModLoaders()) {
            ...
            if (modLoader === null)
                throw new Error("serverAssetsObjects 'modLoader' not null.");
            if (modLoader.modLoaderType === "Forge") {
                minecraftArguments = modLoader.startArguments.arguments;
            ...
            minecraftArguments = this._mojangAssetsGameJsonObjects.arguments;
        }
        if (minecraftArguments == null)
            throw new Error("minecraftArguments not null.");
        array = array.concat(this._minecraftArguments_112later(minecraftArguments));
        return array;
    }
    _minecraftArguments_112later(arg) {
        ...
        for (let i = 0; i < argData.length; i++) {
            ...
            if (argDiscovery.test(argData[i])) {
                let argDataMatch = argData[i].match(argDiscovery);
                ...
                switch (identifier) {
                    ...
                    case "game_directory":
                        ...
                        break;
                    case "assets_root":
                        val = path.join(GlobalPath_1.default.getCommonDirPath(), "assets");
                        break;
            ...
            // forge
            if (this._isModLoaders()) {
                if (argData[i - 1] === "--versionType")
                    val = "Forge";
                if (argData[i - 1] === "--tweakClass")
                    val = "net.minecraftforge.fml.common.launcher.FMLTweaker";
            }
            if (val !== null) {
    ...
    _isAddLog4jXml() {
        if (Utils_1.default.isMcVersion("1.18", this._serverAssetsObjects.minecraftVersion)) {
            return undefined;
        }
    ...
    _getBuildArray_113above() {
        let array = new Array();
        let minecraftArguments = null;
        let argu = this._mojangAssetsGameJsonObjects.arguments;
        // jvm
        if (this._isModLoaders()) {
            const modLoader = this._serverAssetsObjects.modLoader;
            if (modLoader === null)
                throw new Error("serverAssetsObjects 'modLoader' not null.");
            array = array.concat(this._combinationModLoaderJvm(this._jvm_113above(argu.jvm), modLoader.startArguments.arguments.jvm));
        }
        else {
        ...
        if (this._isModLoaders()) {
            ...
            minecraftArguments = argu.game.concat(modLoader.startArguments.arguments.game); // modLoader
            // if (modLoader.modLoaderType === "Forge") {
    ...
    _combinationModLoaderJvm(minecraftJvm, modLoaderJvm) {
        const array = new Array();
    ...
    _minecraftArguments_113later(arg) {
        const argDiscovery = /\${*(.*)}/;
        let array = [];
        ...
        for (let item of argu) {
            if (item.rules !== undefined) {
                ...
                if (item.rules[0].action === "allow") {
                    if (item.rules[0].os.name === Utils_1.default.getOSType()) {
    ...
    _combinationPath() {
        ...
        if (this._isModLoaders()) {
            if (this._serverAssetsObjects.modLoader === null)
                throw new Error("Undefined serverLauncherJsonObjects modLoaders.");
            if (this._serverAssetsObjects.modLoader.modLoaderType === "Forge") {
                if (this._serverAssetsObjects.modLoader.forge === undefined)
                    throw new Error("serverAssetsObjects 'modLoader forge' not null.");
                ...
                libPaths.push(path.join(GlobalPath_1.default.getCommonDirPath(), "libraries", modLoadersVersionJsonObjs.libraries[0].downloads.artifact.path));
            }
            else if (this._serverAssetsObjects.modLoader.modLoaderType === "Fabric") {
                if (this._serverAssetsObjects.modLoader.fabric === undefined)
                    throw new Error("serverAssetsObjects 'modLoader fabric' not null.");
            ...
            libPaths.push(path.join(GlobalPath_1.default.getCommonDirPath(), "versions", this._serverAssetsObjects.minecraftVersion.split("-")[0], `${this._serverAssetsObjects.minecraftVersion.split("-")[0]}.jar`));
        }
        else {
            libPaths = this._mojangAssetsGameJsonObjects.libraries.map((item) => item.filePath);
            libPaths.push(path.join(GlobalPath_1.default.getCommonDirPath(), "versions", this._serverAssetsObjects.minecraftVersion, `${this._serverAssetsObjects.minecraftVersion}.jar`));
        }
        if (Utils_1.default.getOSType() === "windows") {
    ...
    _fabricCombinationLibraries(fabricLibraries, minecraftLibraries) {
    ...
    _forgeCombinationLibraries(forgeLibraries, minecraftLibraries) {
        let libraries = new Array();
        for (let lib of forgeLibraries) {
            if (lib.downloads.artifact.url.length !== 0) {
                libraries.push(path.join(GlobalPath_1.default.getCommonDirPath(), "libraries", lib.downloads.artifact.path));
            }
        }
        for (let lib of minecraftLibraries) {
            if (lib.libType === "artifact") {
...
exports.default = MinecraftStartParameter;
//# sourceMappingURL=MinecraftStartParameter.js.map