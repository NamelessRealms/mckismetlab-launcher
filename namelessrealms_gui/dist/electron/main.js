...
const logger = new LoggerUtil_1.default("ElectronMain");
const isDev = process.env.NODE_ENV === "development";
// const serverUrl = "http://mckismetlab.net:56100";
// const feedUrl = `${serverUrl}/download/latest`;
const feedUrl = `${Configs_1.default.apiUrl}/launcher/autoUpdater/updates`;
...
function initSquirrelAutoUpdater(event) {
...
function createMainWindow() {
    ...
    MainWindow.on("close", () => {
        // save store
        if (MainWindow !== null)
            MainWindow.webContents.send("io", ["save"]);
    });
...
electron.ipcMain.on("openMSALoginWindow", (ipcEvent, args) => {
    ...
    if (isDev)
        MSALoginWindow.webContents.openDevTools();
    MSALoginWindow.on("close", () => {
        if (msaLoginNavigate)
            ipcEvent.reply("MSALoginWindowNotification", "close");
    });
...
electron.ipcMain.on("openMSALogoutWindow", (ipcEvent, args) => {
    ...
    if (MSALogoutWindow == null) {
        ...
        if (isDev)
            MSALogoutWindow.webContents.openDevTools();
        // 禁用選項按鈕
...
function pathCreates(route, serverId) {
    let indexPath;
    if (isDev) {
...
electron.ipcMain.on("windowApi", (event, args) => {
    ...
    if (window === null)
        throw new Error(`windowApi '${args[0]}' null.`);
    switch (args[1]) {
        case "minimize":
            window.minimize();
            break;
